...
pub fn process_record(record: VcfRecord, header: &vcf::Header) -> Vec<String> {
    ...
    if alt_str.len() != 1 {
        eprintln!("More than one alternate base found for record at {}:{}", chr_name, pos);
        std::process::exit(1);
    }

    let alt = alt_str.iter()
        .next()
        .unwrap_or_else(|| {
    ...
    let csq_field_option = info_record.get(
        &header,
        "CSQ"
    );
...
pub fn vep2table(vcf_path: &str, output_path : &str) {
    ...
    let mut reader = vcf::io::reader::Builder::default().build_from_path(vcf_path).unwrap();
